...
class QwenProvider(LLMProvider):
    ...
    def adjudicate_product(self, payload: dict[str, Any]) -> dict[str, Any]:
        prompt = (
            ...
            "Write all descriptive text fields in natural Korean: article_suitability, correction_summary, "
            "reason, and evidence_quote. Do not return English explanatory sentences.\n"
            ...
            "If you say the current product name is incorrect, generic, descriptive, contradicted, or not canonical, "
            "canonical_product_name MUST contain the corrected final display product name supported by evidence; do not leave "
            "canonical_product_name null and do not repeat the incorrect current_product_name. "
    ...
    def adjudicate_exclusive_right(self, payload: dict[str, Any]) -> dict[str, Any]:
        prompt = (
            ...
            "Write all descriptive text fields in natural Korean: reason and evidence_quote. "
            ...
            "If you say the current subject is incorrect, generic, descriptive, sentence-fragment-like, unsupported, "
            "or not canonical, subject_name MUST contain the corrected final exclusive-right subject supported by evidence; "
            "do not leave subject_name null and do not repeat the incorrect current_subject_name. "
            ...
            "Keep the multi-insurer exclusion: if the supplied article context names multiple insurer companies or multiple "
            "exclusive-use-right events, reject or mark ineligible unless a non-multi-company source independently supports this row. "
            "Do not reject or mark ineligible solely because the article title is a roundup, market, management, or column-style "
            "headline when the article is not multi-insurer. If the local evidence names one insurer and one specific "
            "rider/coverage/product/service that acquired or was granted an exclusive-use right, accept it with a corrected subject_name. "
            "Use reject/ineligible_article only when the row should be discarded: ineligible article, non-exclusive-right story, "
            "a multi-insurer/multi-event article without separate non-multi-company support, unrecoverable weak/generic "
            "sentence-fragment subject, or a current subject not supported by the article at all. "
