...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cleanup exclusive-right source records derived from ineligible articles.")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--apply", action="store_true")
    parser.add_argument("--output", default="data/exports/ineligible_article_exclusive_cleanup_plan.csv")
    ...
    with SessionLocal() as db:
        ...
        result = service.apply_exclusive_cleanup(db) if args.apply else {"source_records_excluded": 0, "events_marked": 0}
